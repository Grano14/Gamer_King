...
public class ProdottoDAO {
    ...
    public static Prodotto doRetriveById(String videogioco, String piattaforma){
        try(Connection con = ConPool.getConnection()){
            PreparedStatement ps = con.prepareStatement("select piattaforma, visibilita, datauscita, disponibilita, videogioco from Prodotto where videogioco=? and piattaforma=?");
            ps.setString(1, videogioco);
            ps.setString(2, piattaforma);
            ResultSet rs = ps.executeQuery();
            if(rs.next()){
                Prodotto p = new Prodotto(rs.getString(1), rs.getString(3), rs.getString(5), rs.getBoolean(2), rs.getBoolean(4));
                return p;
            }
            return null;
        }
    ...
    public static void doSave(Prodotto p){
        try(Connection con = ConPool.getConnection()){
            PreparedStatement ps = con.prepareStatement("insert into Prodotto (piattaforma, visibilita, dataUscita, disponibilita, videogioco) values (?,?,?,?,?)");
