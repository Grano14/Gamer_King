...
public class CartaDAO {
    ...
    public static ArrayList<Carta> doRetriveByNomeUtente(String nome){
        ...
        try(Connection con = ConPool.getConnection()){
            ...
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
                Carta c = new Carta(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(6), rs.getString(7),
                        rs.getString(8), rs.getString(9), rs.getString(5),rs.getString(4));
                l.add(c);
            }
            return l;
        }
